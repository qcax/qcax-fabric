...
def main():
    rows=[]
    # Run the repository validator alone first. It recursively scans/compiles the source tree and
    # is the most I/O-sensitive lane. Then run the read-only semantic suite plus four mutation
    # families concurrently. Every mutation family clones into its own private temporary workspace,
    # so cross-family writes cannot touch the admitted source tree.
    ...
    result={'schema':'qcax.aggregate-validation/3','status':'PASS' if not failed else 'FAIL',
